...
Questa manpage è stata originariamente scritta per la distribuzione
Debian GNU/Linux perché non aveva una propria pagina man, ma adesso ho
scritto questa  manpage in formato POD.

B<Muttprint> è un'utility che formatta la stampa di Mutt e di altri
clients di posta come XFMail o Pine per renderla simile alla stampa
ottenuta con Netscape Messenger o Kmail. Può stampare un piccolo
pinguino sulla prima pagina e un titolo in ogni pagina. Soprattutto,
Muttprint stampa solamente gli headers più importanti scartando tutto
ciò che è inutile.
...
semplicemente F<##DOCDIR##/packages/muttprint/sample-muttprintrc>
in F<##SYSCONFDIR##/Muttprintrc> o in F<~/.muttprintrc> se vuoi cambiare le
impostazioni solamente per un utente.

Il linguaggio di default di B<Muttprint> è l'inglese se la variabile
d'ambiente I<LANG> non è specificata. Ad esempio per l'Italiano
dovresti settare:

	export LANG=it_IT

Questo può essere fatto nel tuo F<~/.bashrc>.

Se B<Muttprint> non stampa emettendo il messaggio 
C<lpr: unable to print file: ...>, bisognerà impostarlo per usare
...
descrizione delle impostazioni nell'rcfile è compresa nel file
...
clients come I<XFMail>. Se I<file> è I<->, B<Muttprint> legge dallo
STDIN.
...
Il supporto a CUPS può essere attivato da I<CUPS>; in alternativa puoi usare
...
se vuoi che non venga stampato alcunché.
...
fronte-retro (duplex printing). Di default è 30 secondi.


=item B<-w> I<numero>B<, --wait> I<numero>

Tempo in secondi che B<Muttprint> deve attendere tra la stampa delle
pagine pari e quella delle pagine dispari per la modalità fronte-retro
(duplex printing)

=item B<-F> I<fontname>B<, --font> I<fontname>

Qui puoi scegliere la tua famiglia di font preferita. Le possibilità sono:
...
I</> per la stampa corsiva. Se un header non esiste verrà omesso.
L'ordine saraà lo stesso sullo stampato.
Due esempi:
...
Di default viene usato l'ambiente I<LANG>, e se questi non è impostato
...
Se usi I<original>, la data sarà stampata invariata da come appare
negli headers (la data di invio del messaggio per il sistema che l'ha
inviato e nel formato da questi usato).
Se impostata a I<local>, la data sarà stampata secondo le modalità
...
Per dafault si avrà I<%r E<lt>%aE<gt>,\n>. Soprattutto, puoi
...
Le virgolette sono necessarie perché la stringa contiene spazi,
altrimenti la shell la taglierà in due parti.

=item B<-n> I<String>B<, --verbatimnormal> I<String>

Questo imposta l'opzione che viene passata all'ambiente I<Verbatim>
che è necessaria per stampare il normale testo della mail. Potrai trovare
...
Come per B<--verbatimnormal>, ma questo imposta come verrà stampata
la fimra. Se l'argomento è I<raw>, non ci sarà differenza tra il
...
F</tmp/muttprint.log>. Utile per avere qualche informazione in più del
perché B<Muttprint>, eventualmente, non funzioni.
...
Rimuove la firma (separata da C<-- >) dalla stampa finale. Questo è
...
Questo è software libero; guarda il sorgente per le condizioni di
copia.
Non c'è nessuna garanzia; nemmeno per la commerciabilità o per l'uso in
...
dell'autore è stata realizzata da Roberto Vallone

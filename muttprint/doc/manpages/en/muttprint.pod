...
For instructions how to use B<Muttprint> see 
F</usr/share/doc/packages/muttprint/muttprint-doc-XX.html> where
XX stands for your language.
...
B<CUPS USERS NOTE:> The setting B<PRINTER="lp"> will not
...
Give the short form of your language (currently supported: 
I<de> (German), I<en> (English) and I<it> (Italian) particularly).
...
Remove the quoted paragraph from the printing. B<Muttprint>
detects quoting by following regular expression:

	/^([\t]*[|>:}#])+/

This is the same regexp as B<Mutt> uses so it should work :-)
